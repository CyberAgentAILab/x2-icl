...
def NAN(inst):
    prompt = "Premise: {}\nHypothesis: {}".format(
        inst["premise"].strip(),
        inst["hypothesis"].strip() + ("" if inst["hypothesis"].strip()[-1] in string.punctuation else "."),
    )
...
def ISCS(inst):
    prompt = "Premise: {}\nHypothesis: {}".format(
        inst["prem"].strip(), inst["hypo"].strip() + ("" if inst["hypo"].strip()[-1] in string.punctuation else ".")
    )
...
def QQP(inst):
    prompt = "Question 1: {}\nQuestion 2: {}\nQuestion: Do both questions ask the same thing?".format(
        inst["question1"].strip(), inst["question2"].strip()
    )
    return prompt, meta_dict["QQP"][inst["label"]]
